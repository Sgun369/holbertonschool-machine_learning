...
class Yolo:
    ...
    def non_max_suppression(
        ...
        redundant overlapping boxes with lower scores.
        """
        ...
        for cls in np.unique(box_classes):
            ...
            while len(cls_boxes) > 0:
                max_index = np.argmax(cls_box_scores)
                max_box = cls_boxes[max_index]
                max_score = cls_box_scores[max_index]

                box_predictions.append(max_box)
                predicted_box_classes.append(cls)
                predicted_box_scores.append(max_score)

                cls_boxes = np.delete(cls_boxes, max_index, axis=0)
                cls_box_scores = np.delete(cls_box_scores, max_index, axis=0)

                if len(cls_boxes) == 0:
                    break

                ious = self.iou(max_box, cls_boxes)
                cls_boxes = cls_boxes[ious < iou_threshold]
                cls_box_scores = cls_box_scores[ious < iou_threshold]
    ...
    def iou(self, box1, boxes):
        ...
        (IOU) between a box and a set of boxes.
        """
        x1, y1, x2, y2 = box1
        boxes_x1, boxes_y1, boxes_x2, boxes_y2 = boxes[:,
    ...
    def load_images(folder_path):
        """loads images from a specific folder"""
        image_paths = []
        images = []
